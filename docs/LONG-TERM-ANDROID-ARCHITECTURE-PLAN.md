...
**阶段 3 进度（2026-09-13）：** 默认本地构建入口 `build:web`/`verify:dist` 已切换到组合构建与来源凭据校验，旧脚本保留为 `*:legacy`；独立 `sync-upstream` CLI 完整运行时先绑定锁并应用版本再组合，`validate.yml` 增加锁定 tag 物化步骤。发布工作流复用 build:web，因此发布构建来源也已切换；Git 合并式同步机制仍保留，真实相邻版本迁移验收尚未完成。默认构建依赖本地 `.git` 与锁定 tag，无 Git 的 ZIP 直接 Web 构建不再承诺（Web 产物仅供开发测试）。
...
**阶段三收口（2026-09-13）：** 锁定上游的独立组合构建与同版本等价验证已完成；默认构建入口已接入。验收证据和阶段四/六剩余边界见 [组合构建报告](architecture/composition-build.md)。不据此宣称任意新版本可自动同步。

...
| 来源 | 数量 | 本轮处理 |
| --- | ---: | --- |
| 可重建变换 | 10 | 复用 overlay manifest 的 8 个变换、样式变换和已抽出的更新检查变换；检查重放幂等 |
| 自有文件 | 13 | recipe 明确列出，沿用根目录唯一实现；碰到上游同名文件立即拒绝 |
| 过渡整文件覆盖 | 1 | `app.js` 同时核对上游和本地 SHA-256；任一变化均拒绝，等待人工审查/迁移 |
| 其余发布文件 | 4 | 原版直通 |

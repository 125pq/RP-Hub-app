...
## 2026-09-14：组合同步 1.9.4 回放被旧根目录对照阻塞

- 旧锁：`4aef0bb46c9b3370faba174a20435e5989799727`（1.9.3）；回放目标：`d312bd4b2798dad3f30307afdbd704aac1f80f1a`（1.9.4）。
- 隔离仓首个有效错误：`composed source differs from root: assets/css/styles.css`，桥接测试退出 1。没有 Git 内容冲突；测试错误地要求新版本组合源等于保留的 1.9.3 根目录。
- 修复：根目录等价断言限定迁移基线；当前版本缓存与新鲜组合源逐文件比对。正式 compose 命令及回放统一执行同步、平台、性能门禁后再构建校验。回放 Git 失败不再被忽略。
- 验证入口：`npm run verify:full-sync -- --tag 1.9.4`，报告记录实际运行的门禁；同时保留主仓 1.9.3 同步测试与构建检查。不能仅以 build/verify 通过宣称工作流门禁通过。
- 主仓锁与版本不升级，不发布，不改 GitHub/Gitee；签名 APK 和远程发布链仍需单独验收。

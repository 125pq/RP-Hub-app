...
-2::green::, -1.5::vibrant, colorful, saturated::`,
    ...
    const buildMainModelUiTemplatePrompt = ({ templatePayload, openTag, closeTag, userName }) => [
        ...
        '只允许更新当前变量JSON中已经存在的字段路径，以及变量说明明确允许新增的动态键或ID；除此之外不得新增对象键或顶层变量。判断普通字段是否存在只看本轮提供的当前变量JSON，之前失败输出中出现过的字段不算已创建。动态键必须满足变量说明中的关联条件；人物、背包等数组新增内容必须返回完整数组。',
    ...
    const buildUiTemplateAnalysisSystemPrompt = ({ userInfo, currentVariableJson, variableSchemaText, userName }) => [
        ...
        '只允许更新当前变量JSON中已经存在的字段路径，以及变量说明明确允许新增的动态键或ID；除此之外不得新增对象键或顶层变量。动态键必须满足变量说明中的关联条件，人物、背包等数组新增内容必须返回完整数组。',
